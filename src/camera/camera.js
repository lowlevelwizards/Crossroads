...
(() => {
  ...
  function create({battlefieldViewport:v,battlefieldSurface:s,zoomReadout:z,clamp,onViewChanged=()=>{},onTransformChanged=()=>{},onViewSettled=onViewChanged}){
    ...
    function updateCameraDetailLevel(){const r=relative();document.documentElement.style.setProperty("--camera-relative-zoom",r.toFixed(3));document.documentElement.style.setProperty("--miniature-scale","1");document.body.classList.toggle("camera-far",r<.82);document.body.classList.toggle("camera-normal",r>=.82&&r<1.65);document.body.classList.toggle("camera-close",r>=1.65);document.body.classList.toggle("camera-inspection",r>=2.25);document.body.classList.toggle("camera-rotated",rotated());}
