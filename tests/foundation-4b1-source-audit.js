...
const integration = read("src/rules/shooting-integration.js");
...
check(integration.includes("attemptOrder = function attemptOrderWithPureMorale"), "engine Order Tests are not delegated");
check(integration.includes("resolveShootingCore = function resolveShootingWithPureRules"), "engine shooting is not delegated");
check(integration.includes("commandSupport = function commandSupportWithPureMorale"), "engine command support is not delegated");
check(integration.includes("window.CrossroadsCombatIntegration"), "combat integration diagnostic is missing");
check(validation.includes("CrossroadsMoraleRules"), "startup validation does not require morale rules");
check(validation.includes("CrossroadsCombatIntegration"), "startup validation does not require combat integration");

if (failures.length) {
  console.error("FAIL — Foundation 4B.1 source audit", failures);
  process.exitCode = 1;
} else {
  console.log("PASS — Foundation 4B.1 source ownership and load order are valid.");
}
